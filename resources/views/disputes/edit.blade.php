...
@section('breadcrumb')
    <div class="breadcrumbs-area clearfix">
        ...
        <ul class="breadcrumbs pull-left">
            ...
            @endcanany
            <li><span>{{ __('Edit Dispute') }}</span></li>
        </ul>
    </div>
@endsection
...
@section('content')
        ...
        <div class="col-12">
            <div class="card mt-5">
                <div class="card-header">
                    <h4 class="header-title">{{ __('Edit Dispute')}}
                        ...
                        <div class="form-row">
                                ...
                                <div class="form-group">
                                    <div class="input-group date" id="reported_on" data-target-input="nearest">
                                        <input type="text" id="reportedOn"
                                            ...
                                            name="reported_on" value="{{ Carbon\Carbon::parse($dispute->reported_on)->format('m/d/Y') }}" autocomplete="reported_on"  data-target="#reported_on"
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <select id="beneficiary" aria-describedby="selectbeneficiary"
                                    ...
                                    <option value="{{ $dispute->beneficiary_id }}" selected>
    ...
    <select id="type_of_court" aria-describedby="selecttype_of_court"
        ...
        <option value="land court" @if ($dispute->type_of_court === 'land court') selected="selected" @endif>
            {{ __('Land court') }}
        </option>
        <option value="regional court" @if ($dispute->type_of_court === 'regional court') selected="selected" @endif>
            {{ __('Regional court') }}
        </option>
        <option value="district court" @if ($dispute->type_of_court === 'district court') selected="selected" @endif>
            {{ __('District court') }}
        </option>
        <option value="the court of first instance" @if ($dispute->type_of_court === 'the court of first instance') selected="selected" @endif>
            {{ __('The court of first instance') }}
        </option>
        <option value="court of kadhi" @if ($dispute->type_of_court === 'court of kadhi') selected="selected" @endif>
            {{ __('Court of kadhi') }}
        </option>
        <option value="social welfare/police" @if ($dispute->type_of_court === 'social welfare/police') selected="selected" @endif>
            {{ __('Social welfare/Police') }}
        </option>
        <option value="arbitration commission" @if ($dispute->type_of_court === 'arbitration commission') selected="selected" @endif>
            {{ __('Arbitration commission') }}
        </option>
    </select>
                                        ...
                                        @foreach ($type_of_services as $type_of_service)
                                            <option value="{{ $type_of_service->id }}"
                                                @if ($type_of_service->id === $dispute->type_of_service_id)
                                                    ...
                                                    @endif
                                                >
                                                {{ __($type_of_service->type_of_service) }}</option>
                                        @endforeach
                                    @else
                                        <option>{{ __('No types of services found') }}</option>
                                    @endif
                                </select>
                                        ...
                                        @foreach ($type_of_cases as $type_of_case)
                                            <option value="{{ $type_of_case->id }}"
                                                @if ($type_of_case->id === $dispute->type_of_case_id)
                                                    ...
                                                    @endif
                                                >
                                                {{ __($type_of_case->type_of_case) }}</option>
                                        @endforeach
                                    @else
                                        <option>{{ __('No types of cases found') }}</option>
                                    @endif
                                </select>
                                @error('type_of_case')
                                    <span class="invalid-feedback" role="alert">
...
<script type="text/javascript">
    $(function() {
        ...
        // Run on page load (important for edit mode with old values)
        toggleCourtField();
